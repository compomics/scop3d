                ...
                if not seqID == '':
                    ...
                    if (identityPercent >= float(cutoff)):
                        try:
                            ...
                            f.write(sequence.read() + "\n")
                            if verbose:
                                print(seqID + " (identity " + str(identityPercent) + "% >= cutoff " + str(
                                    cutoff) + "%) - adding")
                            ...
                            for erecord in erecords:
                                r = SeqRecord(Seq.Seq(erecord['TSeq_sequence'], IUPAC.unambiguous_dna),
                                              id=erecord['TSeq_accver'], description=erecord['TSeq_defline'])
    ...
    with open(sequencesFile, 'w') as f:
        ...
        try:
            ...
            for record in records:
                DNAsequences.append(
                    SeqRecord(Seq.Seq(record['TSeq_sequence'], IUPAC.unambiguous_dna), id=record['TSeq_accver'],
                              description=record['TSeq_defline']))
...
def doSplit(sequencesFile, sequencesDir, verbose):
    ...
    subprocess.check_call(
        ['seqretsplit', '-feature', '0', '-sequence', sequencesFile, '-firstonly', '0', '-auto', '-osdirectory2',
         sequencesDir])
    print('OK');
...
def doPairwiseAlignment(sequencesDir, alignmentsDir, verbose):
    ...
    for sequenceFile1 in os.listdir(sequencesDir):
        ...
        for sequenceFile2 in os.listdir(sequencesDir):
            ...
            subprocess.check_call(
                ['needle', '-asequence', sequencePath1, '-bsequence', sequencePath2, '-noendweight', '-endopen', '10.0',
                 '-endextend', '0.5', '-brief', '-aformat', 'srspair', '-auto', '-aname_outfile',
                 sequenceFile1 + '_' + sequenceFile2, '-adirectory_outfile', alignmentsDir])
...
def calcIdentitySimilarity(alignmentsDir, alignmentStatsFile, verbose):
    ...
    with open(alignmentStatsFile, 'w') as stats:
        ...
        for alignmentFile in os.listdir(alignmentsDir):
            ...
            writer.writerow([sequence1, sequence2, identity + '/' + length, identityPercent, similarity + '/' + length,
                             similarityPercent])
    print('OK')
...
def calcConsensus(alignmentFile, consensusFile, verbose):
    ...
    subprocess.check_call(
        ['cons', '-sequence', alignmentFile, '-plurality', '1', '-identity', '0', '-auto', '-outseq', consensusFile,
         '-name', 'consensus'])
    if verbose:
...
def calcVariation(abundancePercentFile, consensusAbundanceVariationFile, verbose):
    ...
    with open(consensusAbundanceVariationFile, 'w') as f:
        ...
        for line in abundancePercentLines[1:]:
            ...
            if verbose:
                print(residue + "\t max abundance: " + str(round(maximum, 1)) + "\t variation: " + str(
                    round(variation, 1)))
    print('OK')
    ...
    for idx, alignment in enumerate(records.alignments):
        for hsp in alignment.hsps:
            ...
            summary = (str(idx + 1) + ": " + str(words[3]) + " - " + str(title[1:100]) + "...\n" +
                       ...
                       "identities: " + str(hsp.identities) + "\t\t" + "positives: " + str(
                        hsp.positives) + "\t\t" + "gaps: " + str(hsp.gaps) + "\n")
    ...
    for ref in protein['dbReferences']:
        if ref['type'] == 'PDB':
            ...
            summary = (str(idx) + ": " + str(ref['id']) + "\t\t" +
                       "resolution: " + str(ref['properties']['resolution']) + "\t\t" + "chains: " + str(
                        ref['properties']['chains']) + "\t\t" +
...
def doChainAlignments(pdbID, structure, consensusFile, chainSequencesDir, verbose):
    ...
    for chain in chains:
        ...
        if verbose:
            ...
            print(sequence)
        subprocess.check_call(
            ['needle', '-asequence', sequenceOutput, '-bsequence', consensusFile, '-noendweight', '-endopen', '10.0',
             '-endextend', '0.5', '-brief', '-aformat', 'srspair', '-auto', '-aname_outfile',
             pdbID + '_' + chain.get_id(), '-adirectory_outfile', chainSequencesDir])
    print('OK')
...
def getChainEntropyVariationData(pdbID, entropyFile, consensusVariationFile, chainSequencesDir, adjustChains, outputDir,
                                 verbose):
                    ...
                    for atom in residue:
                        ...
                        if atom.is_disordered():
                            ...
                            for altloc in atom.disordered_get_id_list():
                                ...
                                if verbose:
                                    print("Chain: " + chainID + "\t residue: " + residueName + "\t atom: " + str(
                                        a.get_full_id()) + " \t b-factor: " + str(a.get_bfactor()) + " => " + str(
                                        value))
                                a.set_bfactor(value)
                        else:
                            if verbose:
                                print("Chain: " + chainID + "\t residue: " + residueName + "\t atom: " + str(
                                    atom.get_full_id()) + " \t b-factor: " + str(atom.get_bfactor()) + " => " + str(
                                    value))
        ...
        for chain in model:
            ...
            if chainID in adjustChains:
                ...
                for residue in chain:
                    ...
                    if verbose:
                        print("Chain: " + chainID + "\t residue: " + residueName + " " + str(
                            sequenceID) + "\t value: " + value)
                    residueID += 1
                    if (residueID >= len(substitutionData[chainID])):
                        break
    print('OK')
    ...
    if sequenceFile != None:
        ...
        for o in organism:
            if o['type'] == 'scientific':
                ...
                break
        record = SeqRecord(Seq.Seq(protein['sequence']['sequence'], IUPAC.protein), id=protein['accession'],
                           name=protein['id'], description=protein['id'])
...
def getUniprotVariants(uniprotID, variantsFile, verbose):
    ...
    if verbose:
        print ("Protein " + variants['accession'] + " " + variants['entryName'] + " has " + str(
            len(variants['features'])) + " features.")
    features = {}
    xrefIndex = {}
...
def getEnsemblVariants(organismName, ensemblID, variantsFile, verbose):
    ...
    with open(variantsFile, 'w') as f:
        subprocess.check_call(
            ['perl', os.path.dirname(os.path.abspath(__file__)) + '/../EnsemblWorker/worker.pl', organismName,
             ensemblID], stdout=f)
    variants = {}
        ...
        for i in range(0, seq_len):
            ...
            for strain in strains:
                ...
                if (aa != '-'):
                    ...
                    cdns[strain] = cdn
            consCdn = "".join([max(counts[0].iteritems(), key=operator.itemgetter(1))[0],
                               max(counts[1].iteritems(), key=operator.itemgetter(1))[0],
                               max(counts[2].iteritems(), key=operator.itemgetter(1))[0]])
            ...
            if freq > 0:
                ...
                ids = "-"
            if verbose:
                print("Position: " + str(i) + " residue: " + consAA + " frequency: " + str(
                    freq) + " variants: " + variants + " type: " + types)
...
def calcFrequencyType(features, sequenceFile, outputFile, verbose):
    ...
    with open(outputFile, 'w') as f:
        ...
        for residueID, residue in enumerate(sequence.seq):
            ...
            ids = ",".join(ids)
            if verbose:
                print("Position: " + str(position) + " residue: " + residue + " frequency: " + str(
                    freq) + " variants: " + change + " type: " + snptype)
